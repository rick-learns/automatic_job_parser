type FilterBarProps = {
  ...
  selectedDiscoveredDate: string | null
  setSelectedDiscoveredDate: (date: string | null) => void
  selectedPostedDate: string | null
  setSelectedPostedDate: (date: string | null) => void
  availableDates: {
    discovered: string[]
    posted: string[]
  }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <select
            ...
            onChange={(e) => setSelectedDiscoveredDate(e.target.value || null)}
            className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50"
          >
            <option value="">All Discovery Dates</option>
            {availableDates.discovered.map(date => (
              <option key={date} value={date}>{date}</option>
            ))}
          </select>
          <select
            value={selectedPostedDate || ''}
            onChange={(e) => setSelectedPostedDate(e.target.value || null)}
            className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50"
          >
            <option value="">All Posted Dates</option>
            {availableDates.posted.map(date => (
              <option key={date} value={date}>{date}</option>
            ))}
          </select>
        </div>
